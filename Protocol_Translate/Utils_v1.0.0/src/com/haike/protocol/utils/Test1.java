...
public class Test1 {
	...
	public void test3() {
		...
		// ApplicationContext ctx = new ClassPathXmlApplicationContext(
		// "ApplicationContext.xml");
		ProtocolTranslateImpl.addProtocol(JOYOUNG.DEVICE_LED_1X.MID, JOYOUNG.DEVICE_LED_1X.PROTOCOL);
		for (int i = 0; i < 100000; i++) {
			// String name = JOYOUNG.DEVICE_LED_1X.MID + i;
			ProtocolTranslateImpl.addProtocol(JOYOUNG.DEVICE_LED_1X.MID + i, JOYOUNG.DEVICE_LED_1X.PROTOCOL);
		}
		...
		for (int i = 0; i < 100000; i++) {
			...
			json3 = ProtocolTranslateImpl.getTranslateJson(JOYOUNG.DEVICE_LED_1X.MID + i, payload3);
		}
	...
	public void testJava2xml() {
		...
		ApplicationContext ctx = new ClassPathXmlApplicationContext("ApplicationContext.xml");
		ProtocolTranslateImpl.addProtocol(JOYOUNG.DEVICE_LED_1X.MID, JOYOUNG.DEVICE_LED_1X.PROTOCOL);
		Protocol protocol = ProtocolTranslateImpl.getProtocol(JOYOUNG.DEVICE_LED_1X.MID);
		File file = new File("src");
		try {
			Protocol2XML.BuildXMLDoc(JOYOUNG.DEVICE_LED_1X.MID, protocol);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
